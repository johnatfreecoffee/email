...
**Status:** Live on Cloudflare Pages. Agent Mail (`~/Documents/AgentMail`) is retired — worker + Settings live here.
...
- Your Supabase project — tables `email_*`, `mc_sessions`, `mc_push_subscriptions`, storage bucket `email-attachments`
...
- Also accepts: valid `mc_sessions` rows
...
- Point Resend inbound at this app's `/api/email/inbound`.
